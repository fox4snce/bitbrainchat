...
const llmPluginSelect = document.getElementById('llm-plugin-select');
...
llmPluginSelect.addEventListener('change', handleLLMPluginChange);
    ...
    if (message) {
        ...
        fetch('/chat', {
            ...
            body: JSON.stringify({ 
                ...
                llm_plugin: llmPluginSelect.value
            }),
        })
...
function loadAvailableLLMPlugins() {
    fetch('/available_llm_plugins')
        .then(response => response.json())
        .then(data => {
            llmPluginSelect.innerHTML = '';
            data.plugins.forEach(plugin => {
                const option = document.createElement('option');
                option.value = plugin;
                option.textContent = plugin;
                llmPluginSelect.appendChild(option);
            });
        })
        .catch(error => {
            console.error('Error:', error);
            updateStatus.textContent = 'Error loading LLM plugins';
        });
}

llmPluginSelect.addEventListener('change', function() {
    const selectedPlugin = this.value;
    fetch('/set_llm_plugin', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
        body: JSON.stringify({ plugin_name: selectedPlugin }),
    })
    .then(response => response.json())
    .then(data => {
        if (data.status === 'success') {
            updateStatus.textContent = `LLM plugin set to ${selectedPlugin}`;
        } else {
    ...
    .catch(error => {
        console.error('Error:', error);
        updateStatus.textContent = 'Error setting LLM plugin';
    });
});

// Function to update the token and cost info with the selected LLM plugin
function updateTokenAndCostInfo(data) {
    tokenInfo.textContent = `Total Input tokens: ${data.input_tokens} | Total Output tokens: ${data.output_tokens}`;
    costInfo.textContent = `Total cost: $${data.total_cost.toFixed(6)} (Using ${llmPluginSelect.value})`;
}

function handleLLMPluginChange() {
    const selectedPlugin = llmPluginSelect.value;
    updateStatus.textContent = `Switching to ${selectedPlugin}...`;
    ...
    .then(data => {
        if (data.status === 'success') {
            updateStatus.textContent = `LLM plugin set to ${selectedPlugin}`;
            // Optionally, you can reload the current conversation here to update token counts
